...
def update_policy(args,
    ...
    if (iteration - 1) % 10 == 0:
        grad_buffer = []
        approx_error = []
        for n_samples in range(100,args.test_size,200):
            grads_s = torch.autograd.grad(get_loss_s(n_samples), policy_net.parameters())
            loss_grad_s = torch.cat([grad.view(-1) for grad in grads_s]).data
            grad_buffer.append(loss_grad_s.cpu().numpy())
            approx_error.append(((loss_grad - loss_grad_s).norm()/loss_grad.norm()).cpu().numpy())
        np.save("batch{}_grad_iter{}.npy".format(args.test_size,iteration),np.array(grad_buffer))
        np.save("batch{}_approx_error_iter{}.npy".format(args.test_size,iteration),np.array(approx_error))
        np.save("batch{}_true_grad_iter{}".format(args.test_size,iteration),loss_grad.cpu().numpy())
    grads_s = torch.autograd.grad(get_loss_s(), policy_net.parameters())
    loss_grad_s = torch.cat([grad.view(-1) for grad in grads_s]).data
    print((loss_grad - loss_grad_s).norm()/loss_grad.norm())
    print(loss_grad_s.norm())
...
def update_params(args, batch, policy_net, value_net, policy_optimizer,
                  likelihood, gp_mll, gp_value_optimizer, nn_value_optimizer,iteration):
    train_size = int(len(batch.state) * 0.7)
    test_size = len(batch.state) - train_size
    ...
    targets = advantages[:train_size] if args.advantage_flag else returns
    if args.pg_estimator == 'BQ':
        ...
        u_fb_t, s_fb_t, v_fb_t = fast_svd.pca_U(args, U_prob.squeeze(-1),
                                                policy_net)
        states_t = states[train_size:]
        actions_t = actions[train_size:]
        action_means_t = action_means[train_size:]
        action_stds_t = action_means[train_size:]
        action_log_stds_t = action_log_stds[train_size:]

        states = states[:train_size]
        actions = actions[:train_size]
        returns = returns[:train_size]
        action_means = action_means[:train_size]
        action_stds = action_means[:train_size]
        action_log_stds = action_log_stds[:train_size]
        state_values_estimates = state_values_estimates[:train_size]
        ...
        GP_inputs = torch.cat([states, args.v_tens[:train_size]], 1)
        args.GP_inputs = GP_inputs

        GP_inputs_t = torch.cat([states_t, args.v_tens[train_size:]], 1)
        args.GP_inputs_t = GP_inputs_t
        ...
        with gpytorch.settings.max_cg_iterations(
            ...
            value_net.eval()
            q_post = likelihood(value_net(GP_inputs_t,
                      state_multiplier=args.state_coefficient,
                      fisher_multiplier=fisher_multiplier))     
            alpha_s = q_post.mean.unsqueeze(-1)
    ...
    def get_loss_s(n_samples=-1):
        action_means_t, action_log_stds_t, action_stds_t = policy_net(states_t)
        log_prob_s = normal_log_density(Variable(actions_t), action_means_t,
                action_log_stds_t, action_stds_t)
        action_value_proxy_s = Variable(
            alpha_s.detach()) if args.pg_estimator == 'BQ' else targets
        pg_loss_s = (-action_value_proxy_s[:n_samples] * log_prob_s[:n_samples]).mean()
    ...
    update_policy(args, get_loss,get_loss_s, get_kl, policy_net, policy_optimizer,
                  value_net, likelihood,iteration)
